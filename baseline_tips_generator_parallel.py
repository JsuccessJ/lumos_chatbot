...
def generate_tip_pair(champion, opponent):
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "너는 리그 오브 레전드 게임의 전문가야. 내가 질문을 하면 웹 검색을 기반하여 가장 정확한 최신 정보를 알려줘."},
                {"role": "user", "content": f"플레이어가 {champion}로 {opponent}를 상대할 때의 플레이 팁을 25단어로 요약해줘."}
            ],
            max_tokens=128,
            temperature=1.0
        )
...
# 챔피언 이름 순서대로 정렬된 딕셔너리 생성
sorted_champion_tips = {champ: champion_tips[champ] for champ in sorted(champion_tips)}
